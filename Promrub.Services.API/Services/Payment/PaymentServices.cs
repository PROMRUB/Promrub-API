...
namespace Promrub.Services.API.Services.Payment
{
    public class PaymentServices : IPaymentServices
    {
        ...
        public async Task<(MemoryStream, string ReceiptNo)> GenerateReceipt(string orgId, string transactionId,bool isImage)
        {
            ...
            var pdfBytes = await NA4Reciept(bytes, org, brn, paymentDetails, paymentItems, couponItems, pos, promptPowered, qrByte,isImage);
        ...
        [Obsolete]
        public async Task<byte[]> PosCarbonReciept(byte[] bytes, OrganizationEntity org, string brn, PaymentTransactionEntity paymentDetails, List<PaymentTransactionItemEntity> paymentItems, PosEntity pos, byte[] promptBytes, byte[] qrByte)
        {
